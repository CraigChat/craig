...
var Ennuizel = (function(ez) {
    ...
    function begin() {
        ...
        return fetch(craig + "?id=" + id + "&key=" + key + "&fetch=info").then(function(res) {
            return res.text();
    ...
    function doDownload(thread, trackNo) {
        ...
        var sock = new WebSocket("wss://" + url.host + "/ennuizel/ws");
    ...
    function connection(sock, thread, trackNo) {
        ...
        if (trackNo in info.tracks) {
            var tinfo = info.tracks[trackNo];
            name = trackNo + "-" + tinfo.name + "#" + tinfo.discrim;
        } else {
            name = trackNo + "";
        }
        newTrack();
