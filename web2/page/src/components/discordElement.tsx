...
export default function DiscordElement({ avatar, icon, name, username, discriminator, discrim, elementType, type }: DiscordElementProps) {
  ...
  return (
    <div class="inline-flex flex-row items-center gap-1 justify-center">
      {elementIcon || ''}
      <span>{username || name}{elementDiscrim ? <span class="text-zinc-400">#{elementDiscrim}</span> : ''}</span>
    </div>
  )
}
