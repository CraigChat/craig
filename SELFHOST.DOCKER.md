...
Run this on your server to download the compose file and a pre-filled environment template:
...
TASMAS_OUTPUT_DIR=        # $CRAIG_RECORDINGS_DIR/tasmas
TASMAS_MODEL_CACHE_DIR=   # $CRAIG_RECORDINGS_DIR/tasmas-model-cache
...
TASMAS watches the recordings directory, transcribes each `.flac.zip` with Whisper, and posts an AI summary to Discord. It runs as part of the default stack — the API keys in step 2 are what activate it.
...
For development (hot reload, build from source), see [docs/development.md](docs/development.md).
