...
<button
  class="flex cursor-pointer items-center gap-2 rounded-md px-3 py-1 text-sm transition-colors hover:bg-white/10 hover:text-white data-[open]:bg-white/10 data-[open]:text-white sm:text-base"
  use:floatingRef
...
>
  <Icon icon={langIcon} />
  <span class="hidden sm:contents">{$t('name')}</span>
...
{#if popupOpen}
  <Portal target="body">
    <div
      bind:this={wrapper}
      use:floatingContent
      class="small-scrollbar z-50 flex max-h-40 flex-col gap-1 overflow-auto rounded-md bg-neutral-700 py-1 pl-1 text-neutral-300 shadow shadow-black"
    ...
    >
      {#each $locales as lang}
        ...
        <button
          class="flex cursor-pointer items-center gap-1 rounded-md px-2 py-1 transition-colors hover:bg-white/25 hover:text-white"
          onclick={() => {
            locale.set(lang);
