...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (parsedFormats.some((f) => !f)) {
    return res.status(400).send({ error: 'Invalid format' });
  }
  if (parsedFormats.length === 0) {
    return res.status(400).send({ error: 'Select at least one format' });
  }
  const selectedFormats: string[] = [];
  for (const parsedFormat of parsedFormats) {
    if (!parsedFormat || selectedFormats.includes(parsedFormat.value)) {
      continue;
    }
  ...
  if (typeof enabled !== 'boolean') {
    return res.status(400).send({ error: 'Invalid enabled state' });
  }

  if (dbUser.driveEnabled !== enabled || dbUser.driveService !== service || dbUser.driveFormats.join(',') !== selectedFormats.join(',')) {
