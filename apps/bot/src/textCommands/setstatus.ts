...
export default class SetStatusCommand extends TextCommand {
  ...
  async run(ctx: CommandContext) {
    ...
    if (type !== 'default' && !ctx.args[1]) {
      return void (await replyOrSend(ctx, 'You need a message for your status!'));
    }

    if (sharding.on) {
      sharding.send('setStatus', { status: ctx.args[0], message: ctx.args[1] });
