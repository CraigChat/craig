...
(async () => {
  ...
  if (!jobName) {
    return void console.log('Usage: yarn run-job <job>', process.argv[1]);
  }
  ...
  if (!jobClass) {
    return void console.error('Failed to import job %s', jobName);
  }

  const jobInstance = new jobClass();

  if (!(jobInstance instanceof TaskJob)) {
    return void console.error('Job %s is not an instance of TaskJob', jobName);
  }
